                ...
                >
                    <div style="display: inline-block; vertical-align: middle;">
                        <span v-if="!arrowExpand">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="var(--vjc-key-color)" viewBox="0 0 256 256" v-if="!open">
                    ...
                    <div style="display: inline-block; margin-left:0.5em;" class="properties">{{ lengthString }}</div>
                </span>
                <div style="display: inline-block; vertical-align: middle;">
                    <div class="hover"  @click="clickEventFilter(data)" v-if="!firstKey && useFilter"> 
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="var(--vjc-key-color)" viewBox="0 0 256 256">
                    ...
                    <div class="hover"  @click="clickEventFilter(data, true)" v-else-if="firstKey && selected_dev_row && useFilter">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="darkred" viewBox="0 0 256 256">
...
const clickEventFilter = (data, reset = false) => {
    ...
    paths.pop();
    let selected_path = Number.isFinite(Number(data.key)) ? paths.join('.') : `${paths.join('.')}.${data.key}`;
...
const getValueStyle = (value) => {
    const type = typeof value;
        switch (type) {
            ...
            case 'boolean':
            return { color: (value ? 'var(--vjc-boolean-color-true)' : 'var(--vjc-boolean-color-false)') };
            case 'object':
